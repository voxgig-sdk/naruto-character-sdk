// Command NarutoCharacter-cli is a CLI + REPL for the NarutoCharacter SDK,
...
const prompt = "naruto-character"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "character clan"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NARUTO_CHARACTER_APIKEY (api key), NARUTO_CHARACTER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NARUTO_CHARACTER_APIKEY carries the API key and
	// NARUTO_CHARACTER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NARUTO_CHARACTER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NARUTO_CHARACTER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNarutoCharacterSDK(opts)

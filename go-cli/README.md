# naruto-character-cli

boru-driven command-line client **and** interactive REPL for the NarutoCharacter
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/naruto-character-cli)
make build

# 2. See usage (words, entities, env vars)
./naruto-character-cli --help

# 3. Provide credentials once, via the environment
export NARUTO_CHARACTER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./naruto-character-cli list character
./naruto-character-cli load 1 character            # {id:1} shorthand
./naruto-character-cli load '{id:1}' character       # explicit match map
./naruto-character-cli list clan

# 5. Override the API base URL for a single call
NARUTO_CHARACTER_BASE=https://api.example.com ./naruto-character-cli list character

# 6. No arguments -> interactive REPL
./naruto-character-cli
naruto-character> list character
naruto-character> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/naruto-character-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/naruto-character-cli list character
   ```
...
```sh
./naruto-character-cli load 1 character          # scalar shorthand for {id:1}
./naruto-character-cli load '{id:1}' character     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `naruto-character>`). Each line is
evaluated as its own boru expression:

```text
$ ./naruto-character-cli
naruto-character> list character
naruto-character> /help
naruto-character> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/naruto-character-cli`. |
...
(`./naruto-character-cli <expr>`) and interactively (the REPL), and expressions compose the

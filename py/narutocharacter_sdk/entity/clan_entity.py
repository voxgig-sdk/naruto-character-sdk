...
from narutocharacter_sdk.narutocharacter_types import (
    Clan,
    ClanListMatch,
)
...
class ClanEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ClanEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Clan]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Clan().list() works with no args.
        if reqmatch is None:
            reqmatch = {}

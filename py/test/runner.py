# NarutoCharacter SDK test runner
...
class NarutoCharacterTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NarutoCharacterTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NarutoCharacterTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NarutoCharacterTestRunner.getenv("NARUTO_CHARACTER_TEST_LIVE")
        override = NarutoCharacterTestRunner.getenv("NARUTO_CHARACTER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NarutoCharacterTestRunner.getenv(key)
        ...
        explain = NarutoCharacterTestRunner.getenv("NARUTO_CHARACTER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if NarutoCharacterTestRunner._test_control is not None:
            return NarutoCharacterTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NarutoCharacterTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NarutoCharacterTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NarutoCharacterTestRunner.load_test_control()
...
def load_env_local():
    NarutoCharacterTestRunner.load_env_local()


def env_override(m):
    return NarutoCharacterTestRunner.env_override(m)


def entity_data(v):
    return NarutoCharacterTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NarutoCharacterTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NarutoCharacterTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NarutoCharacterTestRunner.load_test_control()


def live_client_options():
    return NarutoCharacterTestRunner.live_client_options()


def live_delay_ms():
    return NarutoCharacterTestRunner.live_delay_ms()

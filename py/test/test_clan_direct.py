...
class TestClanDirect:

    def test_should_direct_list_clan(self):
        setup = _clan_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-clan", "live" if setup["live"] else "unit")
        if _skip:
...
def _clan_direct_setup(mockres):

...
import (
	...
	omni "github.com/voxgig-sdk/naruto-character-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NarutoCharacterError); is {
				out := map[string]any{
					"name":    "NarutoCharacterError",
...
func omniCtx(arg any, client *sdk.NarutoCharacterSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NarutoCharacterSDK); is {
			client = live

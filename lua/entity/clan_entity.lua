...
local ClanEntity = {}
ClanEntity.__index = ClanEntity


function ClanEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ClanEntity)
  self._name = "clan"
  self._client = client
...
function ClanEntity:get_name()
  return self._name
end


function ClanEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ClanEntity.new(self._client, opts)
end
...
function ClanEntity:mark_deleted()
  self._deleted = true
end


function ClanEntity:deleted()
  return true == self._deleted
end


function ClanEntity:data_set(args)
  if args ~= nil then
...
function ClanEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ClanEntity:match_set(args)
  if args ~= nil then
...
function ClanEntity:stream(action, args, callopts)
...
function ClanEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
require("naruto-character_types")
...
local NarutoCharacterSDK = {}
NarutoCharacterSDK.__index = NarutoCharacterSDK
...
NarutoCharacterSDK._make_feature = _make_feature


function NarutoCharacterSDK.new(options)
  local self = setmetatable({}, NarutoCharacterSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function NarutoCharacterSDK:options_map()
...
function NarutoCharacterSDK:get_utility()
  return Utility.copy(self._utility)
end


function NarutoCharacterSDK:get_root_ctx()
  return self._rootctx
end


function NarutoCharacterSDK:prepare(fetchargs)
...
function NarutoCharacterSDK:direct(fetchargs)
...
function NarutoCharacterSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function NarutoCharacterSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "NarutoCharacterSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function NarutoCharacterSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function NarutoCharacterSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "NarutoCharacterSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function NarutoCharacterSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = NarutoCharacterSDK.new(sdkopts)
  sdk.mode = "test"

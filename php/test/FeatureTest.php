...
// NarutoCharacter SDK feature test
//
...
class FtCtrl extends NarutoCharacterControl
{
...
class FtClient
{
    ...
    private NarutoCharacterUtility $utility;

    public function __construct(NarutoCharacterUtility $utility, array $options)
    {
    ...
    public function get_utility(): NarutoCharacterUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NarutoCharacterContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NarutoCharacterContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NarutoCharacterContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NarutoCharacterConfig::make_feature($name);
    ...
    private static function buildUrl(NarutoCharacterSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NarutoCharacterError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NarutoCharacterResponse($fetched);
        }
    ...
    private function fail(NarutoCharacterContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NarutoCharacterContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

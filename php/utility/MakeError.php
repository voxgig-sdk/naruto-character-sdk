...
class NarutoCharacterMakeError
{
    public static function call(?NarutoCharacterContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new NarutoCharacterContext([], null);
        }
        $op = $ctx->op ?? new NarutoCharacterOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new NarutoCharacterResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof NarutoCharacterError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new NarutoCharacterError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof NarutoCharacterError) {

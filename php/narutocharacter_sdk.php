...
class NarutoCharacterSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = NarutoCharacterHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = NarutoCharacterHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!NarutoCharacterFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, NarutoCharacterFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return NarutoCharacterUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = NarutoCharacterHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = NarutoCharacterHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = NarutoCharacterHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new NarutoCharacterError($op . "_allow",
                "NarutoCharacterSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = NarutoCharacterHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new NarutoCharacterError("graphql_error",
                "NarutoCharacterSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Clan($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_clan === null) {
                $this->_clan = new ClanEntity($this, null);
            }
            return $this->_clan;
        }
        return new ClanEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new NarutoCharacterSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}

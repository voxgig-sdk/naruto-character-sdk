...
describe('ClanDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new NarutoCharacterSDK({
  ...
  test('direct-list-clan', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-clan', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NARUTO_CHARACTER_TEST_CLAN_ENTID']

...
//     (\`new NarutoCharacterSDK(...)\` / \`NarutoCharacterSDK.test(...)\` become
//     \`NarutoCharacterSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'NarutoCharacterSDK'
...
const TEST_SEED = {"entity":{"character":{"test01":{"id":"test01"}},"clan":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NarutoCharacterSDK(...)\` and any
// \`NarutoCharacterSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NarutoCharacterSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NarutoCharacterSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

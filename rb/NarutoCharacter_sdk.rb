...
require_relative 'NarutoCharacter_types'
...
class NarutoCharacterSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = NarutoCharacterHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NarutoCharacterHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, NarutoCharacterFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    NarutoCharacterUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = NarutoCharacterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = NarutoCharacterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = NarutoCharacterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue NarutoCharacterError => err
    ...
    if fetched.is_a?(Hash)
      status = NarutoCharacterHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NarutoCharacterError.new(
        "graphql_error", "NarutoCharacterSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = NarutoCharacterSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end

...
class ClanDirectTest < Minitest::Test
  def test_direct_list_clan
    setup = clan_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-clan", setup[:live] ? "live" : "unit")
    if _should_skip
...
def clan_direct_setup(mockres)
  ...
  if live
    ...
    client = NarutoCharacterSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,

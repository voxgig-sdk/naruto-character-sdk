...
class ClanEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = NarutoCharacterSDK.test(seed, nil)
    seen = base.Clan(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Clan(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NARUTO_CHARACTER_TEST_CLAN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def clan_basic_setup(extra)
  ...
  entid_env_raw = ENV["NARUTO_CHARACTER_TEST_CLAN_ENTID"]

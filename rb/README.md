# NarutoCharacter Ruby SDK



The Ruby SDK for the NarutoCharacter API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
client = NarutoCharacterSDK.test(testopts, sdkopts)
```
...
raise a `NarutoCharacterError` (a `StandardError` subclass), so wrap
...
Create an instance: `clan = client.Clan`
...
```ruby
# list returns an Array of Clan records (raises on error).
clans = client.Clan.list
```
...
```
rb/
├── NarutoCharacter_sdk.rb       -- Main SDK module
...
The main module (`NarutoCharacter_sdk`) exports the SDK class

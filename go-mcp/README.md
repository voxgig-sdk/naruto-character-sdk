# naruto-character-mcp

[MCP](https://modelcontextprotocol.io) server exposing the NarutoCharacter SDK as
two agent tools — `naruto-character_list` and `naruto-character_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/naruto-character-mcp)
make build

# 2. Provide credentials via the environment
export NARUTO_CHARACTER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user naruto-character \
  -- /absolute/path/to/naruto-character-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./naruto-character-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// naruto-character_list: first page of records
{ "entity": "character" }
{ "entity": "character", "query": { } }

// naruto-character_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/naruto-character-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user naruto-character \
     -- "$PWD"/dist/*/naruto-character-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `naruto-character_list` and `naruto-character_load` tools now appear
   in new sessions. Ask the agent to *"list character using naruto-character"*
   and it calls `naruto-character_list` with `{"entity":"character"}`.
...
```sh
export NARUTO_CHARACTER_APIKEY=sk_live_xxx            # API key
export NARUTO_CHARACTER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `naruto-character_list` tool
...
### Call the `naruto-character_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `naruto-character_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `naruto-character_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NARUTO_CHARACTER_APIKEY` | API key sent with every request. |
| `NARUTO_CHARACTER_BASE` | Optional override of the API base URL. |
...
```sh
./naruto-character-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"naruto-character_load","arguments":{"entity":"character","query":{"id":1}}}}'
```

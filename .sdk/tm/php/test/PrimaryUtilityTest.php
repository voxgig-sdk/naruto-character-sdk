...
// NarutoCharacter SDK primary utility test
//
...
//   NarutoCharacterOmni::unwrap, which throws the err so omni can match it
...
//   builds the typed context with NarutoCharacterOmni::ctx_from_map, runs the
//   utility, and writes the observable state back with
//   NarutoCharacterOmni::sync_ctx BEFORE throwing any error - that map is
...
class PrimaryUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = NarutoCharacterOmni::makeRunner(
                self::TEST_JSON_FILE, NarutoCharacterSDK::test(null, null));
    ...
    private static function client(): NarutoCharacterSDK
    {
    ...
    private static function make_test_ctx(
        NarutoCharacterSDK $client,
        NarutoCharacterUtility $utility,
        ?array $overrides = null
    ): NarutoCharacterContext {
        $ctxmap = [
            'opname' => 'load',
            'client' => $client,
            'utility' => $utility,
    ...
    public function test_done_basic(): void
    {
        ...
        $this->runsection('done', function (array $ctxmap) use ($client, $utility) {
            $ctx = NarutoCharacterOmni::ctx_from_map($ctxmap, $client, $utility);
    ...
    public function test_make_error_basic(): void
    {
        ...
        $this->runsection('makeError', function ($ctxmap = null, $errmap = null) use ($client, $utility) {
            $ctx = NarutoCharacterOmni::ctx_from_map(
                is_array($ctxmap) ? $ctxmap : [], $client, $utility);
            $err = NarutoCharacterOmni::err_from_map(is_array($errmap) ? $errmap : null);
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new NarutoCharacterResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new NarutoCharacterBaseFeature();
    ...
    {
        $hook_client = NarutoCharacterSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends NarutoCharacterBaseFeature {
            public $hook_fn = null;
            public function TestHook(NarutoCharacterContext $ctx): void
            {
    ...
    {
        $init_client = NarutoCharacterSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends NarutoCharacterBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new NarutoCharacterSDK([
    ...
    {
        $blocked_client = new NarutoCharacterSDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_fetch_def_basic(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new NarutoCharacterSpec([
            'base' => 'http://localhost:8080',
            'prefix' => '/api',
            'path' => 'items/{id}',
            'suffix' => '',
    ...
    public function test_make_request_basic(): void
    {
        ...
        $this->runsection('makeRequest', function (array $ctxmap) use ($client, $utility) {
            ...
            NarutoCharacterOmni::sync_ctx($ctxmap, $ctx);
            if ($err !== null) {
                throw $err;
            }
            return null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new NarutoCharacterSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = NarutoCharacterSDK::test(null, is_array($setup) ? $setup : null);
        $spec_utility = $spec_client->get_utility();
        $utility = self::client()->get_utility();

        $this->runsection('makeSpec', function (array $ctxmap) use ($spec_client, $spec_utility, $utility) {
            $ctx = NarutoCharacterOmni::ctx_from_map($ctxmap, $spec_client, $spec_utility);
    ...
    public function test_make_url_basic(): void
    {
        ...
        $this->runsection('makeUrl', function (array $ctxmap) use ($client, $utility) {
            ...
            return NarutoCharacterOmni::unwrap(($utility->make_url)($ctx));
        });
    }
    ...
    public function test_operator_basic(): void
    {
        $this->runsection('operator', function ($vin = null) {
            ...
            $op = new NarutoCharacterOperation($vin);
            return [
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = NarutoCharacterSDK::test(null, is_array($setup) ? $setup : null);
        ...
        $this->runsection('prepareAuth', function (array $ctxmap) use ($auth_client, $auth_utility, $utility) {
            $ctx = NarutoCharacterOmni::ctx_from_map($ctxmap, $auth_client, $auth_utility);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runsection('resultBasic', function (array $ctxmap) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof NarutoCharacterError)

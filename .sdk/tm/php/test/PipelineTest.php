...
// NarutoCharacter SDK pipeline test
//
...
{
    private static function utility(): NarutoCharacterUtility
    {
        return new NarutoCharacterUtility();
    }
    ...
    private static function ctx(array $over = []): NarutoCharacterContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new NarutoCharacterContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new NarutoCharacterOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof NarutoCharacterError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new NarutoCharacterOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = NarutoCharacterMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new NarutoCharacterOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = NarutoCharacterMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new NarutoCharacterOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = NarutoCharacterMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NarutoCharacterSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = NarutoCharacterMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new NarutoCharacterResponse([]),
            'result' => new NarutoCharacterResult([]),
        ]);
        [, $err] = NarutoCharacterMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new NarutoCharacterSpec([]),
            'response' => null,
            'result' => new NarutoCharacterResult([]),
        ]);
        [, $err] = NarutoCharacterMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new NarutoCharacterSpec(['step' => 's']),
            'response' => new NarutoCharacterResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new NarutoCharacterResult([]),
        ]);
        [, $err] = NarutoCharacterMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new NarutoCharacterSpec(['step' => 's']),
            'response' => new NarutoCharacterResponse(self::resp(200, ['v' => 1])),
            'result' => new NarutoCharacterResult([]),
        ]);
        [, $err] = NarutoCharacterMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new NarutoCharacterSpec(['step' => 's']),
            'response' => new NarutoCharacterResponse(self::resp(200, ['v' => 2])),
            'result' => new NarutoCharacterResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        NarutoCharacterMakeResponse::call($ctx);
    ...
    {
        $preset = new NarutoCharacterResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new NarutoCharacterSpec([]),
            'response' => new NarutoCharacterResponse([]),
            'result' => new NarutoCharacterResult([]),
        ...
        [$got, $err] = NarutoCharacterMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new NarutoCharacterResult([])]);
        [, $err] = NarutoCharacterMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new NarutoCharacterSpec([]), 'result' => null]);
        [, $err] = NarutoCharacterMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NarutoCharacterSpec(['step' => 's']),
            'result' => new NarutoCharacterResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new NarutoCharacterOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NarutoCharacterMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NarutoCharacterSpec(['step' => 's']),
            'result' => new NarutoCharacterResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new NarutoCharacterOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NarutoCharacterMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NarutoCharacterResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new NarutoCharacterSpec([]),
            'result' => new NarutoCharacterResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = NarutoCharacterMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NarutoCharacterMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new NarutoCharacterError('boom', 'boom');
        $utility->fetcher = function (NarutoCharacterContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NarutoCharacterSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = NarutoCharacterMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (NarutoCharacterContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = NarutoCharacterMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(NarutoCharacterResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        NarutoCharacterMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (NarutoCharacterContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NarutoCharacterSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = NarutoCharacterMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new NarutoCharacterResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new NarutoCharacterSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = NarutoCharacterMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = NarutoCharacterMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = NarutoCharacterMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new NarutoCharacterResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, NarutoCharacterDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new NarutoCharacterResult(['ok' => false])]);
        $this->expectException(NarutoCharacterError::class);
        NarutoCharacterDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new NarutoCharacterResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', NarutoCharacterMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        NarutoCharacterMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new NarutoCharacterBaseFeature();
        $b = new NarutoCharacterBaseFeature();
        NarutoCharacterFeatureAdd::call($ctx, $a);
        NarutoCharacterFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): NarutoCharacterBaseFeature
    {
        $f = new NarutoCharacterBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        NarutoCharacterFeatureAdd::call($ctx, self::named_feature('a'));
        NarutoCharacterFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        NarutoCharacterFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        NarutoCharacterFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        NarutoCharacterFeatureAdd::call($ctx, $replace);
        ...
        NarutoCharacterFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): NarutoCharacterContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new NarutoCharacterSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NarutoCharacterPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        NarutoCharacterPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new NarutoCharacterResponse(['headers' => null]),
            'result' => new NarutoCharacterResult([]),
        ]);
        NarutoCharacterResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        NarutoCharacterResultBody::call($ctx);

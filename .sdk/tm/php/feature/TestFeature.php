...
// NarutoCharacter SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class NarutoCharacterTestFeature extends NarutoCharacterBaseFeature
{
    ...
    public function init(NarutoCharacterContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (NarutoCharacterContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = NarutoCharacterParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (NarutoCharacterContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(NarutoCharacterContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = NarutoCharacterParam::call($ctx, $k);

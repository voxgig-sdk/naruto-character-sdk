...
// NarutoCharacter SDK retry feature
...
class NarutoCharacterRetryFeature extends NarutoCharacterBaseFeature
{
    ...
    public function init(NarutoCharacterContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (NarutoCharacterContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(NarutoCharacterContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(NarutoCharacterContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

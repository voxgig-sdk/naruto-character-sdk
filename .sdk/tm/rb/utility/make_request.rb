...
module NarutoCharacterUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NarutoCharacterResponse.new({})
    ...
    if fetch_err
      ...
      response = NarutoCharacterResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NarutoCharacterResponse.new(fetched)
    else

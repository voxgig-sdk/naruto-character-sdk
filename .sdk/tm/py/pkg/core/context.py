...
from projectname_sdk.core.control import NarutoCharacterControl
...
class NarutoCharacterContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = NarutoCharacterControl()
        ...
        if isinstance(sp, NarutoCharacterSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, NarutoCharacterResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, NarutoCharacterResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return NarutoCharacterOperation({})
    ...
    def make_error(self, code, msg):
        return NarutoCharacterError(code, msg, self)

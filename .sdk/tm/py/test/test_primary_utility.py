...
from projectname_sdk import NarutoCharacterSDK
from projectname_sdk.core.spec import NarutoCharacterSpec
from projectname_sdk.core.result import NarutoCharacterResult
from projectname_sdk.core.response import NarutoCharacterResponse
from projectname_sdk.core.operation import NarutoCharacterOperation
from projectname_sdk.core.error import NarutoCharacterError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import NarutoCharacterBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return NarutoCharacterError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_done_basic(self):
        ...
        def subject(entry):
            ...
            except NarutoCharacterError as e:
                return None, e
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = NarutoCharacterResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = NarutoCharacterSDK.test(None, None)
        ...
        class TestInitFeature(NarutoCharacterBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(NarutoCharacterBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_live(self):
        ...
        live_client = NarutoCharacterSDK({
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = NarutoCharacterSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = NarutoCharacterSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = NarutoCharacterSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = NarutoCharacterOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = NarutoCharacterSDK.test(None, setup_opts)
